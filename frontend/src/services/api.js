...
const api = axios.create({
  baseURL: env.API_BASE, // SIN /api al final
...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    const tokenProblem =
      status === 401 &&
      (code === "token_not_valid" || code === "user_inactive" || code === "authentication_failed");
    ...
    if (tokenProblem && !original._retry) {
      ...
      isRefreshing = true;
      try {
        const r = await axios.post(`${env.API_BASE}/api/auth/refresh/`, { refresh }); // ? Agregados backticks
